...
export default function LoginPage() {
    ...
    if (isSignInWithEmailLink(auth, window.location.href)) {
      ...
      if (emailForSignIn) {
        ...
        signInWithEmailLink(auth, emailForSignIn, window.location.href)
          ...
          .catch(() => {
            toast({ variant: "destructive", title: "Link Expired", description: "Please try logging in again." });
          })
          .finally(() => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      if (!window.recaptchaVerifier) {
        throw new Error("Security check failed. Refresh page.");
      }
  ...
  return (
    <div className="container mx-auto px-4 py-10 flex items-center justify-center min-h-[85vh] relative overflow-hidden">
      ...
      <div className="w-full max-w-[480px] bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl border border-gray-100 relative z-10 animate-fade-in">
        ...
        {/* Hidden Container for ReCAPTCHA - Critical for invisible mode */}
        ...
        <div className="text-center space-y-4 mb-10 flex flex-col items-center">
          ...
          <p className="text-gray-500 font-medium text-sm mt-4">Login or Signup</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="relative py-2 flex items-center">
            ...
            <span className="flex-shrink mx-4 text-[10px] uppercase tracking-widest font-black text-gray-300">OR</span>
            <div className="flex-grow border-t border-gray-100"></div>
          </div>
          ...
          {!isOtpSent ? (
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4 animate-in slide-in-from-bottom-2">
              <div className="space-y-2">
                <p className="text-xs font-bold text-center text-gray-400 uppercase tracking-widest">Enter OTP</p>
                <div className="relative">
          ...
          <div className="relative py-2 flex items-center">
            <div className="flex-grow border-t border-gray-100"></div>
            <span className="flex-shrink mx-4 text-[10px] uppercase tracking-widest font-black text-gray-300">Email</span>
            <div className="flex-grow border-t border-gray-100"></div>
          </div>
          ...
          {!isLinkSent ? (
            <form onSubmit={handleMagicLinkLogin} className="space-y-3">
              ...
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Send Login Link"}
              </Button>
            </form>
          ) : (
            <div className="p-6 bg-primary/5 rounded-3xl text-center space-y-4 border border-primary/10 animate-in zoom-in-95">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">We sent you a login link.</p>
              </div>
        ...
        <div className="flex items-center justify-center gap-4 pt-8 opacity-60">
          <div className="flex items-center gap-1 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            <ShieldCheck className="w-3 h-3 text-primary" /> 100% Secure
          </div>
          <div className="w-1 h-1 bg-gray-200 rounded-full" />
          <div className="flex items-center gap-1 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            <Sparkles className="w-3 h-3 text-secondary" /> Trusted
          </div>
        </div>
      </div>
    </div>
