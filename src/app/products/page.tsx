...
import { ChevronDown, ShoppingBag, X, Share2, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-6 py-12 space-y-12">
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => (
            <Button
              ...
              className={cn(
                "rounded-full px-4 md:px-6 h-10 md:h-12 text-xs md:text-sm transition-all",
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="glass gap-2 rounded-full w-full md:w-auto min-w-[200px] justify-between h-12">
                <span className="text-xs md:text-sm">Sort By: {getSortLabel()}</span> <ChevronDown className="w-4 h-4" />
              </Button>
            </DropdownMenuTrigger>
      ...
      {(searchParam || collectionParam || categoryParam !== 'All' || sortParam !== 'recommended') && (
        <div className="flex items-center gap-4 animate-fade-in bg-white/10 p-4 rounded-2xl border border-white/20 overflow-x-auto">
          <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap">Active Filters:</span>
          <div className="flex gap-2 flex-1">
            {searchParam && <Badge className="bg-primary text-white px-3 py-1 rounded-full whitespace-nowrap text-[10px]">"{searchParam}"</Badge>}
            {collectionParam && <Badge className="bg-secondary text-white px-3 py-1 rounded-full whitespace-nowrap text-[10px]">{collectionParam}</Badge>}
            {categoryParam !== 'All' && <Badge className="bg-accent text-white px-3 py-1 rounded-full whitespace-nowrap text-[10px]">{categoryParam}</Badge>}
          </div>
          <Button variant="ghost" size="sm" onClick={() => router.push('/products')} className="h-9 rounded-xl gap-2 hover:bg-destructive/10 hover:text-destructive shrink-0">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="space-y-4">
              <Skeleton className="aspect-[4/5] rounded-[2rem] md:rounded-[2.5rem]" />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8">
            {paginatedProducts.map((p) => (
              <Link href={`/products/${p.id}`} key={p.id} className="group">
                <div className="glass rounded-[2rem] p-2 md:p-3 space-y-3 md:space-y-4 transition-all hover:scale-[1.02] hover:shadow-2xl hover:shadow-primary/5">
                  <div className="relative aspect-[4/5] rounded-[1.5rem] overflow-hidden bg-muted">
                    ...
                    {p.inventory <= 5 && p.inventory > 0 && (
                      <Badge className="absolute top-2 left-2 md:top-4 md:left-4 bg-orange-500 text-white font-black text-[8px] md:text-[10px] px-2 py-0.5">
                        Low Stock
                      </Badge>
                    )}
                    {p.inventory === 0 && (
                      <Badge className="absolute top-2 left-2 md:top-4 md:left-4 bg-red-500 text-white font-black text-[8px] md:text-[10px] px-2 py-0.5">
                        Sold Out
                      </Badge>
                    )}
                    ...
                    <button 
                      ...
                      className="absolute top-2 right-2 md:top-4 md:right-4 w-8 h-8 md:w-10 md:h-10 rounded-full glass flex items-center justify-center hover:bg-primary hover:text-white transition-all z-10"
                    >
                      <Share2 className="w-3.5 h-3.5 md:w-4 md:h-4" />
                    </button>

                    <div className="absolute inset-x-0 bottom-0 p-2 md:p-4 bg-gradient-to-t from-black/60 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                      <Button 
                        disabled={p.inventory === 0}
                        onClick={(e) => handleQuickAdd(e, p)} 
                        className="w-full bg-white text-black hover:bg-white/90 rounded-lg md:rounded-xl gap-2 h-10 md:h-12 text-xs md:text-sm font-bold"
                      >
                        <ShoppingBag className="w-4 h-4" /> Quick Add
                      </Button>
                    </div>
                  </div>
                  <div className="px-1 md:px-2 pb-2 flex justify-between items-start">
                    <div className="flex-1 min-w-0 pr-2">
                      <h3 className="font-bold text-sm md:text-lg group-hover:text-primary transition-colors truncate">{p.name}</h3>
                      <p className="text-[9px] md:text-[11px] text-muted-foreground font-black uppercase tracking-widest mt-0.5">{p.category}</p>
                    </div>
                    <div className="flex flex-col items-end shrink-0">
                      {p.discountPrice && p.discountPrice > 0 ? (
                        <>
                          <span className="text-[9px] md:text-[10px] text-muted-foreground line-through decoration-muted-foreground/50">Rs.{p.price.toLocaleString()}</span>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-4 py-8">
              <Button
                variant="outline"
                size="icon"
                className="rounded-xl glass h-12 w-12"
